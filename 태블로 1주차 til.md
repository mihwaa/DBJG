...
시트를 불러오고 나면 라이브/추출을 우상단에서 확인할 수 있다.
데이터의 유형 또한 선택할 수 있다.  
시트 연결 후 필드에 따른 유형 (메타 데이터 그리드)의 유형을 변경할 수 있다.  
오른쪽 데이터 그리드에서도 할 수 있다. 

> **🧞‍♀️ 라이브와 추출의 차이가 무엇인가요? 어떤 경우 사용하나요?**


```
 라이브는 데이터에 직접 연결해 원본에 영향을 바로 미친다.
 추출은 추출파일이 따로 저장된다. 라이브에 비해 속도가 빠르다.
```
...
관계로 결합하기 위해서는 결합하려는 데이터의 유형이 동일해야 한다. (각 테이블에서 같은 값을 가지고 있어야 함)  
결합을 사용할 때 필드가 고유값이 아니면 데이터가 중복하여 들어갈 수 있다.  
루트테이블을 삭제하면 이어진 모든 데이터가 삭제된다.  

> **🧞‍♀️ 어떤 경우에 관계를, 어떤 경우에 조인을 사용하나요?**

```
관계는 데이터를 결합할 때 원 데이터의 세부 수준을 유지해주기 떄문에, 데이터를 결합하기 적합한 방법이다.
다만 의도적 필터링, 복제를 위해 조인을 사용하는 경우가 있다.
```
...
<!-- 데이터결합 및 혼합에 대해 알게 된 점을 적어주세요 --> 혼합은 데이터를 실제로 결합하지는 않는다.  
여러 데이터 원본의 혼합된 결과만을 나타낸다.  
두 테이블의 데이터를 하나의 시각화 자료로 활용할 때 사용한다.  
관계로 하는 것이 추천되며, 기술적 한계가 있을 때만 사용을 권한다.


## 7강. 데이터 결합과 유니온

<!-- 유니온에 대해 알게 된 점을 적어주세요 -->
한 테이블의 행을 다른 데이터에 추가하여 결합하는 것.  
따라서 필드 수가 같고 이름이나 유형이 일치해야 한다.

> **🧞‍♀️ 유니온을 사용하기 위한 전제 조건은 무엇인가요?**
> 필드 수가 같고 이름이나 유형이 일치해야 한다.
...
> 라이브는 서버에 있는 데이터가 실시간으로 업데이트되고 추출은 새로 고쳐주어야 업데이트된다.
> 추출은 대규모 데이터 세트, 필터, 계산 등이 포함된 복잡한 시각화에서 효과적이다. (속도 측면)
...
같은 필터를 적용해도 데이터 형식에 따라 다른 화면이 출력됨.  
문자열 > 카테고리 숫자 > 범위 등  
...
```
필드값의 이름이나 데이터 유형, 개수가 모두 같고, 여러 데이터를 결합하는 유니온이 적합하다.
```
...
```
매 순간 변동하는 실시간 데이터를 필요로 하고, 그 즉시 서버에 업데이트가 필요하다면 라이브 연결이 적합하다.
```
...
```
미리 다운로드해서 오프라인으로 활용해야 하거나, 규모가 큰 데이터를 사용해야 할 때 추출이 더 유용할 수 있다.
```
